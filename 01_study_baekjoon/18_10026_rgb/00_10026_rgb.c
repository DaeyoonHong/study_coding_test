...
int main(void)
{
    ...
    scanf("%d", &size_of_map);
    printf("DEBUG: size_of_map = %d\n", size_of_map);
    
    size_of_graph = size_of_map * size_of_map;
    printf("DEBUG: size_of_graph = %d\n", size_of_graph);
    
    printf("DEBUG: Starting input loop\n");
    for (int i = 0; i < size_of_map; i++)
    {
        scanf("%100s", map[i]);
        printf("%d ", size_of_map);
    }
    printf("\nDEBUG: Input loop completed\n");
    
    printf("DEBUG: About to malloc\n");
    st_node **graph = (st_node **)malloc(sizeof(st_node *) * size_of_graph);
    printf("DEBUG: First malloc done, graph = %p\n", (void*)graph);
    st_node **graph_rg = (st_node **)malloc(sizeof(st_node *) * size_of_graph);
    printf("DEBUG: Second malloc done, graph_rg = %p\n", (void*)graph_rg);
    
    printf("DEBUG: Starting initialization loop\n");
    for (int i = 0; i < size_of_graph; i++)
    {
        graph[i] = NULL;
        graph_rg[i] = NULL;
    }
    printf("DEBUG: Initialization loop completed\n");
    
    printf("DEBUG: About to call build_graph\n");
